...
problems.MapGet("/", async (string? topic, Difficulty? difficulty, AppDbContext db, CancellationToken ct) =>
{
    ...
    if (!string.IsNullOrWhiteSpace(topic)) query = query.Where(x => x.TopicTags.Contains(topic.ToLower()));
    if (difficulty is not null) query = query.Where(x => x.Difficulty == difficulty);
...
problems.MapPut("/{id:guid}", async (Guid id, ProblemRequest request, AppDbContext db, CancellationToken ct) =>
{
    ...
    p.Title = request.Title.Trim(); p.ExternalUrl = request.ExternalUrl; p.TopicTags = request.TopicTags.Select(x => x.Trim().ToLower()).Distinct().ToArray(); p.Difficulty = request.Difficulty;
...
api.MapPost("/entries", async (CreateEntryRequest request, ClaimsPrincipal principal, AppDbContext db, CancellationToken ct) =>
{
    ...
    if (problem is null) return Results.ValidationProblem(new Dictionary<string, string[]> { ["problemBankId"] = ["Problem does not exist."] });
...
api.MapGet("/entries", async (DateTimeOffset? from, DateTimeOffset? to, string? topic, ClaimsPrincipal principal, AppDbContext db, CancellationToken ct) =>
{
    ...
    if (from is not null) query = query.Where(x => x.LoggedAt >= from);
    if (to is not null) query = query.Where(x => x.LoggedAt <= to);
    if (!string.IsNullOrWhiteSpace(topic))
    {
        var topicFilter = JsonSerializer.Serialize(new { topic_tags = new[] { topic.ToLower() } });
...
record VerifyEmailRequest(Guid UserId, string Token);
record ProblemRequest(string Title, string ExternalUrl, string[] TopicTags, Difficulty Difficulty)
{
    public Problem ToEntity() => new() { Title = Title.Trim(), ExternalUrl = ExternalUrl, TopicTags = TopicTags.Select(x => x.Trim().ToLower()).Distinct().ToArray(), Difficulty = Difficulty };
}
record CreateEntryRequest(Guid ProblemBankId, int TimeSpentMinutes, int HintsUsed, int? SelfRatedConfidence, DateTimeOffset? ReviewDueAt, DateTimeOffset? LoggedAt);
record SystemDesignRequest(string? WeakTopic, string? Level);
